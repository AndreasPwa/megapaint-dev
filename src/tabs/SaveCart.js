...
class CartList extends React.Component {
  ...
  fetchCarts(token) {
    fetch(api_url + "cart/saved/get", {
      ...
      .then((response) => response.json())
      .then((json) => {
        this.setState({ data: json.data, package: json.items, refreshing: false });
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  decreaseQuantity(id, quantity) {
    ...
    AsyncStorage.getItem("token").then((value) => {
      ...
      fetch(api_url + "cart/update-item", {
        method: "PUT",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({ cart_detail_id: id, type: "minus" }),
      })
        .then((response) => response.json())
        .then((json) => {
          json.status == "success"
  ...
  increaseQuantity(id, quantity) {
    ...
    AsyncStorage.getItem("token").then((value) => {
      ...
      fetch(api_url + "cart/update-item", {
        ...
        body: JSON.stringify({ cart_detail_id: id, type: "plus" }),
      })
        .then((response) => response.json())
        .then((json) => {
          json.status == "success"
  ...
  deleteItem(id) {
    ...
    AsyncStorage.getItem("token").then((value) => {
      ...
      fetch(api_url + "cart/remove-item", {
        method: "DELETE",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({ cart_detail_id: id }),
      })
        .then((response) => response.json())
        .then((json) => {
          json.status == "success"
  ...
  renderRightActions = (progress, dragX, id, i) => {
    return (
      <View
        style={{
          marginVertical: 5,
  ...
  renderListItem = ({ item, index }) => (
    <Swipeable
      renderRightActions={(progress, dragX) =>
        this.renderRightActions(progress, dragX, item.id, index)
      }
            ...
            <View style={styles.fixToText}>
              {item.product_image ? (
                <View style={styles.leftText}>
                  <Image source={{ uri: item.product_image }} style={styles.logoImg} />
                </View>
              ) : (
                <View style={styles.leftText}>
              ...
              <View style={styles.rightText}>
                ...
                <View style={{ marginVertical: 10 }}>
                  <Text style={{ color: "#262c76", fontFamily: "Lexend", fontSize: 16 }}>{"Ks. " + item.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</Text>
                </View>

                <View style={{ flexDirection: "row" }}>
                  <Text style={{ alignSelf: "center", paddingRight: 5, fontSize: 18, fontFamily: "Lexend" }}>{"Quantity: "}</Text>
                  <TouchableOpacity onPress={() => this.decreaseQuantity(item.id, item.quantity)}>
                    <Image
                  ...
                  <TouchableOpacity onPress={() => this.increaseQuantity(item.id, item.quantity)}>
                    <Image
                      style={{ height: 25, width: 25 }}
            ...
            <View style={{ flex: 1 }}>
              ...
              <View style={styles.checkoutView}>
                <View style={styles.fixToText}>
                  ...
                  <Text style={styles.rightText}>{this.state.data.total_price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}{" Ks"}</Text>
                </View>
                <View style={styles.fixToText}>
                  <Text style={styles.leftText}>{"Taxes:  "}</Text>
                  <Text style={styles.rightText}>{this.state.data.tax_amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}{" Ks"}</Text>
                </View>
                <View style={styles.fixToText}>
                  <Text style={styles.leftText}>{"Total:  "}</Text>
                  <Text style={styles.rightText}>{(this.state.data.total_price + this.state.data.tax_amount).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}{" Ks"}</Text>
                </View>
